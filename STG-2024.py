...
if not st.session_state.logged_in:
    ...
    if st.session_state.first_login:
        ...
        if st.session_state.password_expired:
            ...
            page = st.sidebar.radio('Select page', ['STG-2024', 'View Logs', 'Add New Item'])
            ...
            if page == 'STG-2024':
                ...
                actual_quantity = st.number_input('Actual Quantity', min_value=0, step=1)

                if st.button('Add Item'):
                    if item_name and actual_quantity:
                        c.execute("INSERT INTO materials (item_name, actual_quantity) VALUES (?, ?)", (item_name, actual_quantity))
                        conn.commit()
                        st.success('Item added successfully!')
                    else:
                        st.error('Please enter both item name and quantity.')
